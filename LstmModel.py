...
dropout_prob = 0.2
save_path = r'./models/lstmModel2.ckpt'
...
if __name__ == '__main__':
    ...
    pred_lstm_values = predictions_by_model[0].squeeze(2)
    lstm_values = np.ravel(inverse_transform(y_test.values.reshape(-1, 1), pred_lstm_values))
    ...
    subplots_time_series(df_lstm_values.index.to_list(), df_lstm_values["total_load_real_values"],
                         df_lstm_values["total_load_predicted_values"], "LSTM")
    plot_multiple_time_series(df_lstm_values.index.to_list(), df_lstm_values["total_load_real_values"],
                              df_lstm_values["total_load_predicted_values"], "LSTM")


def record():
    df = pd.DataFrame({'lstm': train_losses})  # 创建dataframe
    df1 = pd.DataFrame({'lstm': df_lstm_values["total_load_predicted_values"]})  # 创建dataframe
    df.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\loss\lstm.csv', index=False)
    df1.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\pred\lstm.csv', index=False)
