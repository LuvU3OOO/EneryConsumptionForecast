...
dropout_prob = 0.2
save_path = r'./models/BilstmModel2.ckpt'
...
def train():
    for epoch in range(n_epochs):
        ...
        torch.save(LstmNet.state_dict(), save_path)
...
def forecast(history, n_seq):
    ...
    with torch.no_grad():
        pred = LstmNet(torch.Tensor(input_x).to(device))
...
if __name__ == '__main__':
    ...
    print('BiLstm training time:', training_time)
    plot_losses()
    ...
    summarize_scores('LstmNet', score, scores)
    pred_vanilla_lstm_values = predictions_by_model[0].squeeze(2)
    lstm_values = np.ravel(inverse_transform(y_test.values.reshape(-1, 1), pred_vanilla_lstm_values))
    ...
    subplots_time_series(df_lstm_values.index.to_list(), df_lstm_values["total_load_real_values"],
                         df_lstm_values["total_load_predicted_values"], "BiLSTM")
    plot_multiple_time_series(df_lstm_values.index.to_list(), df_lstm_values["total_load_real_values"],
                              df_lstm_values["total_load_predicted_values"], "BiLSTM")


def record():
    df = pd.DataFrame({'bilstm': train_losses})  # 创建dataframe
    df1 = pd.DataFrame({'bilstm': df_lstm_values["total_load_predicted_values"]})  # 创建dataframe
    df.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\loss\bilstm.csv', index=False)
    df1.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\pred\bilstm.csv', index=False)
