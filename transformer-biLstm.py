...
dropout = 0.2
d_model = 32
save_path = r'./models/TfModelbilstm.ckpt'
...
if __name__ == '__main__':
    ...
    subplots_time_series(df_transformer_values.index.to_list(), df_transformer_values["total_load_real_values"],
                         df_transformer_values["total_load_predicted_values"], "Transformer-bilstm")
    plot_multiple_time_series(df_transformer_values.index.to_list(), df_transformer_values["total_load_real_values"],
                              df_transformer_values["total_load_predicted_values"], "Transformer-bilstm")


def record():
    df = pd.DataFrame({'Transformer-Bilstm': train_losses})  # 创建dataframe
    df1 = pd.DataFrame({'Transformer-Bilstm': df_transformer_values["total_load_predicted_values"]})  # 创建dataframe
    df.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\loss\t-bil.csv', index=False)
    df1.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\pred\t-bil.csv', index=False)
