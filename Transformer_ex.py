...
dropout = 0.2
d_model = 64
save_path = r'./models/TfModel2.ckpt'
...
class Transformer(nn.Module):
    def __init__(self):  # d_model 表示特征维度（必须是head的整数倍）, num_layers 表示 Encoder_layer 的层数， dropout 用于防止过你和
        ...
        self.decoder = nn.Linear(32, 1)  # 这里用全连接层代替了decoder， 其实也可以加一下Transformer的decoder试一下效果
        self.init_weights()
...
def record():
    df = pd.DataFrame({'transformer': train_losses})  # 创建dataframe
    df1 = pd.DataFrame({'transformer': df_transformer_values["total_load_predicted_values"]})  # 创建dataframe
    df.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\loss\transformer.csv', index=False)
    df1.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\pred\transformer.csv', index=False)
