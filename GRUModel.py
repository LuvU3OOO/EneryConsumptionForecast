...
dropout_prob = 0.2
save_path = r'./models/GruModel2.ckpt'
...
def train():
    ...
    torch.save(GruNet.state_dict(), save_path)
...
def forecast(history, n_seq):
    ...
    with torch.no_grad():
        pred = GruNet(torch.Tensor(input_x).to(device))
...
if __name__ == '__main__':
    ...
    print('Gru training time:', training_time)
    plot_losses()
    ...
    summarize_scores('GruNet', score, scores)
    pred_Gru_values = predictions_by_model[0].squeeze(2)
    Gru_values = np.ravel(inverse_transform(y_test.values.reshape(-1, 1), pred_Gru_values))
    print(Gru_values.shape)
    print(Gru_values[:10])
    df_Gru_values = format_predictions(Gru_values, y_test, y_test.index)
    print(df_Gru_values.head())
    subplots_time_series(df_Gru_values.index.to_list(), df_Gru_values["total_load_real_values"],
                         df_Gru_values["total_load_predicted_values"], "Gru")
    plot_multiple_time_series(df_Gru_values.index.to_list(), df_Gru_values["total_load_real_values"],
                              df_Gru_values["total_load_predicted_values"], "Gru")


def record():
    df = pd.DataFrame({'gru': train_losses})  # 创建dataframe
    df1 = pd.DataFrame({'gru': df_Gru_values["total_load_predicted_values"]})  # 创建dataframe
    df.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\loss\gru.csv', index=False)
    df1.to_csv(r'C:\D\PytorchProgect\pytorchrtest\EnergyForecast\data\pred\gru.csv', index=False)
